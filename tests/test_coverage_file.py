...
def test_coverage_file_initialization(temp_coverage_file, mock_coverage_data, mock_file_observer):
    ...
    # Need to mock the FileWatcher and FILE_OBSERVER globals
    with patch("python_coverage.FileWatcher") as mock_watcher, \
         patch("python_coverage.FILE_OBSERVER", mock_file_observer):
        from python_coverage import CoverageFile

        cov_file = CoverageFile(temp_coverage_file)

        assert cov_file.coverage_file == temp_coverage_file
        assert cov_file.data is not None
        mock_file_observer.schedule.assert_called_once()
...
def test_coverage_file_update(temp_coverage_file, mock_coverage_data, mock_file_observer):
    ...
    with patch("python_coverage.FileWatcher"), \
         patch("python_coverage.FILE_OBSERVER", mock_file_observer):
        from python_coverage import CoverageFile
